if (!window.LOGIN) {
  window.LOGIN = {};
  LOGIN.addOnload = function(func) {
    var existingOnload = window.onload;
    return window.onload = function() {
      var error;
      try {
        existingOnload && existingOnload.apply(this, arguments);
      } catch (e) {
        error = e;
      }
...
LOGIN.module = (function() {
  ...
  ret = {
    ...
    setCacheKey: function setCacheKey(ck) {
      cacheKey = ck;
    },
...
LOGIN.module.interceptLogout = function() {
  var libraryURL = "http://www.library.upenn.edu/";
  var plugURL = "https://weblogin.library.upenn.edu/cgi-bin/PLUG2?authz=pennlin&app=" + encodeURIComponent(libraryURL) + "&logout=logout";
  var head = document.head ? document.head : document.getElementsByTagName('head')[0];
  var script = document.createElement("script");
  script.setAttribute('src', plugURL);
