 ...
 */
function pennlib_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL) {
  ...
  $form['external_libraries']['semantic_ui'] = array(
    ...
    In order to enable, download the plugin at <a href="https://github.com/Semantic-Org/Semantic-UI" target="_blank">https://github.com/Semantic-Org/Semantic-UI</a> and place the uncompressed directory in the pennlib root directory and rename it to "semantic_ui".<br>
The path should be <b>pennlib/semantic_ui/</b>. You can also cd into pennlib and run git clone https://github.com/Semantic-Org/Semantic-UI.git semantic_ui && git clone https://github.com/artberri/sidr.git sidr'),
...
In order to enable, download the plugin at <a href="https://github.com/artberri/sidr" target="_blank">https://github.com/artberri/sidr</a> and place the uncompressed directory in the pennlib root directory and rename it to "sidr".<br>
The path should be <b>pennlib/libraries/sidr/</b>. You can also cd into pennlib/libraries/ and run git clone https://github.com/Semantic-Org/Semantic-UI.git semantic_ui && git clone https://github.com/artberri/sidr.git sidr'),
    '#default_value' => theme_get_setting('sidr'),
  );

  // display messages to the user if any library is missing
  if (theme_get_setting('sidr')) {
    $sidr_path_js = drupal_get_path('theme', 'pennlib') . '/libraries/sidr/dist/jquery.sidr.js';
    $sidr_path_css = drupal_get_path('theme', 'pennlib') . '/libraries/sidr/dist/stylesheets/jquery.sidr.light.css';
    if (file_exists($sidr_path_js) && file_exists($sidr_path_css)) {
      drupal_set_message(t('Sidr is properly installed!'));
    }
    else {
      drupal_set_message(t('Sidr is not properly installed yet. You must download the plugin <a href="https://github.com/artberri/sidr" target="_blank">here</a> and place the uncompressed directory in the pennlib theme root folder.'), 'error');
    }
  }
  if (theme_get_setting('semantic_ui')) {
    $semantic_ui_path_js = drupal_get_path('theme', 'pennlib') . '/libraries/semantic_ui/dist/semantic.js';
    $semantic_ui_path_css = drupal_get_path('theme', 'pennlib') . '/libraries/semantic_ui/dist/semantic.css';
    if (file_exists($semantic_ui_path_js) && file_exists($semantic_ui_path_css)) {
      ...
      drupal_set_message(t('Semantic UI is not properly installed yet. You must download the plugin <a href="https://github.com/Semantic-Org/Semantic-UI" target="_blank">here</a> and place the uncompressed directory in the pennlib theme root folder.'), 'error');
    }
  }
